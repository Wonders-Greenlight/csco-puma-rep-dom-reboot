...
const config = {
    ...
    CORS: {
        allowedOrigins: [
            process.env.HOST, 
            "*", 
            "http://greenlight-csco-puma.myshopify.com", 
            "https://greenlight-csco-puma.myshopify.com", 
            `http://${process.env.APP_URL.replace(/https:\/\//, '')}`, 
            `http://${process.env.APP_URL.replace(/https:\/\//, '')}`, 
            `http://${process.env.HOST.replace(/https:\/\//, '')}`, 
            `https://${process.env.HOST.replace(/https:\/\//, '')}`, 
            ...(process.env.CORS || 'https://nine-unlike-comfort-start.trycloudflare.com,https://greenlight-csco-puma.myshopify.com').split(',')
        ]
    },
    JWT: {
