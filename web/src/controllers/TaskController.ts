...
import TaskProvider, {
  ...
  TASKFILE_BY_TYPE,
  TASK_FUNCTION,
} from "../providers/TaskProvider.js";
import {
  AppMode,
  ObjectValues,
  PartialBy,
} from "../interfaces/AppInterfaces.js";
...
class TaskController {
  public async registerTask(task: PartialBy<ITask, "filePath" | "fnToCall">) {
    try {
      const newTask = new Task(task);
      const tempRoute = path.resolve(
        folders.TEMP,
        `${task.type}_${newTask._id}.json`
      );
  ...
  public async getTaskById(req: Request, res: Response) {
    ...
    try {
      const startResults =
        Number(page) === 1 ? 0 : Number(limit) * (Number(page) - 1);
        ...
        if (
          ...
          task.innerData = {
            resources: innerData?.resources?.filter((x: any) =>
              ...
              task.outerData.results?.some(
                (y: any) => String(y.erpId) === String(x.erpId)
              )
            ),
          };
        }
  ...
  public async deleteTask(req: Request, res: Response) {
    ...
    try {
      ...
      if (
        typeof result.innerData === "string" &&
        fs.existsSync(result.innerData)
      ) {
        fs.unlinkSync(result.innerData);
      }

      const existOnBus = TaskProvider.queue.findIndex(
        (x) => x.id === result.busId
      );
      if (existOnBus !== -1) {
  ...
  public async checkErrorOrders(nowDate: Date | string): Promise<void> {
    ...
    tasks.forEach(async (task) => {
      const existEnqueuedOrderTask = TaskProvider.queue.find(
        (x) => x.id === task.busId && x.type === task.type
      );
