...
import { AddHandlersParams, BillingInterval, LATEST_API_VERSION, LogSeverity, RegisterReturn, Session } from '@shopify/shopify-api';
...
import {
    ...
    GqlProductInput, GqlStagedUploadInput, GqlInventoryAdjustItemInput, GqlAdjustInventoryInput, 
    Transaction, CreateCarrierServiceParams, GqlMediaInput, CreateFulfillmentParams, CreateFulfillmentEventParams
...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb+srv://neatsoull:12345@cluster0.if84o.mongodb.net/?appName=Cluster0';
...
class ShopifyProvider {
    ...
    static REST_MS_LIMIT_PER_SECOND: number = 1000 / ShopifyProvider.REST_LIMIT_AMOUNT_PER_SECOND
    ...
    private async readyChecker( rebuild: boolean = false ): Promise<void> {
        ...
        try {
            ...
            console.log(err)
            console.error('ERROR WHILE CREATING SHOPIFY API')
        }
    }
    ...
    private async throttleApiRequests( firstCallWait: boolean = false ) {
        ...
        if ( !!!ShopifyProvider.LAST_REST_API_CALL ) {
            ...
            if ( timeSinceLastCall < ShopifyProvider.REST_MS_LIMIT_PER_SECOND ) {
                timeToWait = ShopifyProvider.REST_MS_LIMIT_PER_SECOND - timeSinceLastCall
            }
        }
    ...
    public throttleRest( fn: any, delay: number = ShopifyProvider.REST_MS_LIMIT_PER_SECOND ) {
    ...
    public validateHmac( query: object ) {
        ...
        const validate = crypto.createHmac('sha256', this.API_PASSWORD)
    ...
    public async getMetafieldsByOwner({ id, resourceType, clearSession = false }: GetMetafieldByOwnerParams): Promise<RestResources['Metafield'][]> {
        ...
        if ( clearSession ) 
            metafields.data = ShopifyProvider.sessionRemover<RestResources['Metafield']>(metafields.data)
    ...
    public async getProductsByHandles( handles: string, fields: string = '', clearSession: boolean = true ) {
    ...
    public async getAllProducts({ fields = [] }: any, clearSession: boolean = true ) {
        ...
        do {
            const response: any = await this.queueRestRequest(async () =>
    ...
    public async getProducts({ 
        ...
        published_status = '',
        page_info = ''
    }: any, clearSession: boolean = true ) {
        const params: any = {
            handle, fields, limit, collection_id, created_at_max,
            created_at_min, published_at_max, published_at_min,
            updated_at_max, updated_at_min, presentment_currencies,
            product_type, since_id, status, title, vendor,
            published_status
        }
    ...
    public async getProductById( id: string | number, fields: string = '', clearSession: boolean = true ) {
    ...
    public async getProductsCount( clearSession: boolean = true ) {
        const count = await shopify.api.rest.Product.count({ session: await this.getOfflineSession() })

        if ( clearSession ) delete count.session
        return count
    }

    public async getProductVariantsCount( productId: string | number, clearSession: boolean = true ) {
    ...
    public async deleteProductImage( productId: string | number, imageId: string | number, clearSession: boolean = true ) {
    ...
    public async createOrUpdateProduct(payload: any, clearSession: boolean = true ) {
    ...
    public async getOrderById( id: string | number, clearSession: boolean = true ): Promise<RestResources['Order']> {
    ...
    public async updateOrderById( id: number, fields: any, clearSession: boolean = true ): Promise<RestResources['Order']> {
    ...
    public async getPendingOrders( fields: string[] = [], clearSession: boolean = true ) {
    ...
    public async getOrders({ 
        ...
        processed_at_max = '',
        processed_at_min = ''
    }: any, clearSession: boolean = true ): Promise<RestResources['Order'][]> {
        await this.readyChecker()
    ...
    public async getLocations( clearSession: boolean = true ): Promise<RestResources['Location'][]> {
        await this.readyChecker()
        const session = await this.getOfflineSession()

        const locations = await shopify.api.rest.Location.all({ session })

        if ( clearSession ) locations.data = ShopifyProvider.sessionRemover(locations.data)
        return locations.data
    }

    public async getLocationById( id: string | number, clearSession: boolean = true ) {
        await this.readyChecker()
    ...
    public async getCarrierServices( clearSession: boolean = true ) {
        ...
        if ( clearSession ) carriers.data = ShopifyProvider.sessionRemover(carriers.data)
        return carriers.data
    }
    
    public async getCarrierServiceById( id: number, clearSession: boolean = true ): Promise<RestResources['CarrierService']> {
        await this.readyChecker()
        const session = await this.getOfflineSession()
    ...
    public async createCarrierService( params: CreateCarrierServiceParams, clearSession: boolean = true ) {
        await this.readyChecker()
        const session = await this.getOfflineSession()
    ...
    public async getOrderTransactions( orderId: string | number, clearSession: boolean = true ): Promise<Transaction[]> {
        await this.readyChecker()
        const session = await this.getOfflineSession()

        const transactions = await shopify.api.rest.Transaction.all({ session, order_id: +orderId })

        if ( !clearSession ) return transactions.data
        return ShopifyProvider.sessionRemover(transactions.data)
    }

    public async getFulfillmentOrdersByOrderId( orderId: number, clearSession: boolean = true ): Promise<RestResources['FulfillmentOrder'][]> {
        await this.readyChecker()
    ...
    public async createFulfillment( payload: CreateFulfillmentParams, clearSession: boolean = true ): Promise<RestResources['Fulfillment'] | any> {
    ...
    public async createFulfillmentEvent( payload: CreateFulfillmentEventParams, clearSession: boolean = true ) {
    ...
    public async getAllInventoryLevels(
        ...
        do {
            ...
            const response: any = await this.queueRestRequest(async () =>
                await shopify.api.rest.InventoryLevel.all(params)
            )
    ...
    public async getInventoryItemById(id: number | string, clearSession: boolean = true ) {
        await this.readyChecker()
        const session = await this.getOfflineSession()
    ...
    public async setAvailableInventoryLevel({
        ...
        if ( !!!inventoryItem ) throw { message: `Inventory item not found with id: ${inventoryItemId}` }
    ...
    public async getCustomerById( customerId: string | number, clearSession: boolean = true ): Promise<RestResources['Customer']> {
    ...
    public async updateCustomerById( id: number, fields: any, clearSession: boolean = true ): Promise<RestResources['Order']> {
    ...
    public async getWebhookById( id: number, clearSession: boolean = false ): Promise<RestResources['Webhook']> {
    ...
    public async registerWebhookHandlers( session?: Session ): Promise<RegisterReturn> {
    ...
    public async gqlOrderMarkAsPaid( order: RestResources['Order'], gqlClient?: any ) {
        if ( !gqlClient ) {
        ...
        try {
            ...
            const orderData = (orderMarkAsPaid.body as any).data.orderMarkAsPaid.order
            
            if ( !!!orderData ) {
                throw {
                    response: {
                        errors: (orderMarkAsPaid.body as any).data.orderMarkAsPaid.userErrors
                    }
                }
            }
    ...
    public async gqlOrderEditBegin( order: RestResources['Order'], gqlClient?: any ) {
        if ( !gqlClient ) {
            gqlClient = this.createGraphqlClient()
        }
    ...
    public async gqlCreateOrUpdateProduct(
        ...
        const mutationKey: keyof typeof GQL_MUTATIONS = !!productInput.id ? 'UPDATE_PRODUCT' : 'CREATE_PRODUCT'
        ...
        try {
            ...
            if ( typeof productInput.images !== 'undefined' ) delete queryData.variables.media
            ...
            const { product, userErrors } = (createUpdateProduct.body as any).data[gqlKey]
    ...
    public async gqlGetProductsByQuery(
        query: string,
        gqlClient?: any
    ) {
        if ( !!!gqlClient ) {
        ...
        try {
            ...
            ShopifyProvider.GRAPHQL_API_AVAILABLE_CREDITS = extensions.cost.currentlyAvailable
    ...
    public async gqlCreateStagedUploads(
        ...
        try {
            ...
            const stagedItemsData = (createStagedUploads.body as any).data.stagedUploadsCreate.stagedTargets
            const userErrors = (createStagedUploads.body as any).data.stagedUploadsCreate.userErrors
    ...
    public async gqlAdjustQuantities(
        ...
        try {
            ...
            const bulkInventoryUpdate = (inventoryAdjustQuantities.body as any).data.inventoryAdjustQuantities
            const userErrors = (inventoryAdjustQuantities.body as any).data.inventoryAdjustQuantities.userErrors
    ...
    public async gqlGetOrdersByQuery(
        query: string,
        gqlClient?: any
    ) {
        if ( !!!gqlClient ) {
