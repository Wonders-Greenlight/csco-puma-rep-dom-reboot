    ...
    try {
      ...
      for await (const order of halfProcessedOrders) {
        try {
          ...
          await order.pushLog(
            `Cron checker: Error while iterating => ${err.message}`,
            LogType.WARNING
          );
      ...
      HelpersController.sendSlackMessage(
        "Cron Order Checker - (Not Iteration)",
        [
          {
            type: "section",
            fields: [
  ...
  public async allocateOrderInvoices({ nowDate }: { nowDate: string }) {
    ...
    for (let i = 0; i < orders.length; i++) {
      ...
      try {
        ...
        console.error(
          "ERROR ALLOCATING ORDER - SHOPIFY API GETTING ORDER ERROR"
        );
      ...
      try {
        ...
        const someError = _response.orderDetailsResult.find(
          (x: any) => typeof x.payload !== "undefined"
        );
