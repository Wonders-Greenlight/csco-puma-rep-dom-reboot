...
import { IEvent, ITask, InnovateProductInfo, InnovateVariantInfo, TASK_SUCCESS_MESSAGE, TaskState, TaskType } from '../interfaces/TaskInterfaces.js'
import { GqlImage, GqlInventoryQuantity, GqlMetafield, GqlProductInput, GqlProductStatus, GqlVariant, GqlWeightUnit, MetafieldOwner, RestResources, WeightUnits } from '../interfaces/ShopifyInterfaces.js'
...
import { ActionPayload, CustomShopifyItemProduct, CustomShopifyItemProductVariant, ERP_ACTION, ShopifyItemProduct, ShopifyItemProductInventory } from '../interfaces/ErpInterfaces.js';
...
class ProductTasksController extends BaseTaskController {
    ...
    private cleanGqlInput( obj: any ): GqlProductInput {
        Object.entries(obj).forEach(([key, value]) => {
            if ( value !== undefined ) {
                ...
                if ( Array.isArray(value) ) obj[key] = value.map(x => this.cleanGqlInput(x))
    ...
    private addErpIdParamProcess<T extends { [key: string]: any; }>( erpProducts: T[] ): (T & {
        erpId: string;
    })[] {
        // HOMOLOGATE THIS
    ...
    private async processParams( 
        ...
        const tagKeys = [
            'Color', 'CategoryName', 'DepartmentName', 'Size',
            'Size', 'Color', 'BinLocation', 'CategoryName', 'DepartmentName',
        ]
            ...
            try {
                const thisProduct = ProductTasksController.shopifyProducts.find(p => {
                    const variantMatch = p.variants.some((v: any) => 
                        v.sku === product.variants[0].ItemLookupCode
                    )
                ...
                const payload: InnovateProductInfo = {
                    ...
                    status: thisProduct?.status?.toUpperCase() as GqlProductStatus || appCfg.productCreationMode as GqlProductStatus || GqlProductStatus.ACTIVE,
                    type: product.type,
                ...
                if ( !!!thisProduct && task.type !== TaskType.CREATE_UPDATE_PRODUCTS ) continue

                const variants: InnovateVariantInfo[] = [];

                // CHECK IF WE NEED TO ADD THIS PRODUCT PAYLOAD, CHECKING LastUpdated and microstate from last task
                const thisProductResult = lastTopicTask?.outerData?.results.find((r: any) => 
                    r.erpId === payload.erpId
                )

                const _variants = product.variants.filter(variant => {
                    // CHECK IF WE NEED TO ADD THIS PRODUCT PAYLOAD, CHECKING LastUpdated and microstate from last task
                    const isoUpdated = new Date(variant.LastUpdated)
                    isoUpdated.setHours(isoUpdated.getHours() + 4)
                    const updatedAfterLastTask = isoUpdated > lastTopicTask?.startedAt
                    if ( !!thisProductResult?.status && !updatedAfterLastTask && !!thisProduct) {
                ...
                _variants.forEach(variant => {
                    ...
                    if ( !!thisProduct ) {
                        ...
                        const thisVariant = thisProduct.variants.find((v: any) => v.sku === variant.ItemLookupCode)
                        if ( !!thisVariant ) {
                        ...
                        if ( !!!thisVariant && !!thisProduct && task.type !== TaskType.CREATE_UPDATE_PRODUCTS ) {
                            return
                        }
                    }
                ...
                Object.entries({ ...product, ...(product.variants.at(0) || {}) }).forEach(([key, value]) => {
    ...
    // ------- END PRIVATE
    public async getErpProducts( task?: ITask ): Promise<CustomShopifyItemProduct[]> {
        ...
        const pFields = []
        if ( [TaskType.UPDATE_PRICE, TaskType.UPDATE_STOCK].includes(task.type as any) ) {
            pFields.push(
                'ItemLookupCode',
                'Reference',
                'Size',
        ...
        try {
            if ( task.type === TaskType.CREATE_UPDATE_PRODUCTS ) {
                ...
                const erpProducts = await Agilisa.getAllMasterProducts({ fields: pFields })
            ...
            const masterInventory = await Agilisa.getAllMasterInventory({ fields, limit: 3000 })
            ...
            for await (const product of groupedProductsInventory) {
                if ( task.type === TaskType.UPDATE_PRICE ) {
                    product.variants = product.variants.reduce((acc: CustomShopifyItemProductVariant[], v: ShopifyItemProductInventory) => {
    ...
    public async checkVariantsInventory( product: InnovateProductInfo ) {
        ...
        const thisProduct = ProductTasksController.shopifyProducts.find(x => 
        ...
        for await ( const variant of product.variants ) {
            ...
            const thisVariant = thisProduct?.variants.find((v: any) => 
                v.id === variant.shopifyId || v.admin_graphql_api_id === variant.shopifyId
            )
            if ( !!!thisVariant ) continue

            try {
                await HelpersController.sleep(200)
                const variantInventoryLevels = await ShopifyProvider.queueRestRequest(async () => 
                    await ShopifyProvider.getInventoryLevels({ 
                        inventory_item_ids: String(thisVariant.inventory_item_id)
                    })
                )

                const missingLevels = variant.inventory.filter(x => 
                    !variantInventoryLevels.some((l: any) => l.location_id === x.locationId)
                )
                ...
                for ( const level of missingLevels ) {
                    ...
                    await ShopifyProvider.queueRestRequest(async () => 
    ...
    public async checkAllVariantInventories({ products }: { products: InnovateProductInfo[] }) {
        ...
        for (const product of products) {
            ...
            for await ( const variant of product.variants ) {
                ...
                try {
                    const variantInventoryLevels = allInventoryLevels.filter(inv => 
                        inv.inventory_item_id === thisVariant.inventory_item_id
                    )
    ...
    public async createUpdateProducts({ nowDate }: { nowDate: string }) {
        ...
        try {
            ...
            // ------------------ POOL PROCESSING
            /* V2 IDEA --- */
            // EVALUATE CHANGING THIS "POOL FEATURE" WITH A "WHILE" CONDITION,
            // AND SPLICING VALUES EACH NEW PROMISE ALL PROCESS, SO WE CAN
            // TAKE AN AVERAGE REQUEST TIME RESPONSE AND UPDATE THE POOL SIZE, ON EACH POOL PROCESSING
            // AND NOT DEFINE THE POOLS ONCE AND THEN PROCESS, JUST GO SPLICING DOWN PER POOL FROM THE
            // MAIN PRODUCTS ARRAY.
            
            // let initialTotalCredits: null | number = null
            // let queryCost: null | number = null
            // let restoreRate: null | number = null
            // let SAFE_RETURNING_CREDITS: null | number = null
            // let poolSize: null | number = null
            // let requestTime: null | number = null // on ms

            // const firstProduct = products.shift()
            // console.log('PRODUCTS AFTER SHIFT ==>', products.length)

            // const firstProductInfo: any = {
            //     erpId: firstProduct.erpId,
            //     shopifyId: 0,
            //     observations: '',
            ...
            // const firstGqlPayload = this.buildCreateUpdateGqlParams(firstProduct)
            // // CHECK TIME TOOK REQUEST TO GO BACK
            // const firstRequestTimeStart = Date.now()
            // const { product: firstProductData, cost: _cost } = await ShopifyProvider.gqlCreateOrUpdateProduct(firstGqlPayload, gqlClient)
            // const firstRequestTimeEnd = Date.now()
            // requestTime = firstRequestTimeEnd - firstRequestTimeStart
            // const secondsRequestTime = Math.floor((firstRequestTimeEnd - firstRequestTimeStart) / 1000)
            // console.log('REQUEST TIME (ms) ===>', requestTime)
            // console.log('PRODUCT RESPONSE ===>', firstProductData)
            // // CHECK TIME TOOK REQUEST TO GO BACK
            // initialTotalCredits = _cost.throttleStatus.maximumAvailable
            // restoreRate = _cost.throttleStatus.restoreRate // USE THIS ALSO AFTER KNOWING REQUEST TIME
            // queryCost = _cost.actualQueryCost
            // SAFE_RETURNING_CREDITS = secondsRequestTime * restoreRate
            // poolSize = Math.floor(SAFE_RETURNING_CREDITS / queryCost)

            // console.log(_cost)
            // console.log(SAFE_RETURNING_CREDITS)
            // console.log(poolSize)

            // // log finish product on task
            // firstProductInfo.apiHealthData = _cost
            // firstProductInfo.observations = 'Product created successfully'
            // firstProductInfo.shopifyId = firstProductData.id
            // firstProductInfo.processedAt = Date.now()
            // result.success++
            // result.products.push(firstProductInfo)

            // const restOfProductsPools = HelpersController.groupArrayIntoPools(products, poolSize)

            // console.log('TOTAL POOLS', restOfProductsPools.length)
            // for await (const productPool of restOfProductsPools) {
            //     console.log('CURRENT POOL', productPool.length)
                
            //     await Promise.all(
            //         productPool.map(async _product => {
            //             const pInfo: any = {
            ...
            //                 const { product, cost } = await ShopifyProvider.gqlCreateOrUpdateProduct(gqlPayload, gqlClient)
            ...
            for (let i = 0; i < products.length; i++) {
                ...
                const thisProduct = ProductTasksController.shopifyProducts.find(x => 
                    x.id === _product.shopifyId || x.admin_graphql_api_id === _product.shopifyId
                )
                ...
                try {
                    ...
                    if ( isUpdating ) {
                        ...
                        if ( task.type === TaskType.CREATE_UPDATE_PRODUCTS && !!_product.variants ) {
                        ...
                        if ( task.type === TaskType.UPDATE_PRICE ) {
                            let prodNeededKeys: Array<keyof InnovateProductInfo> = [
                                'shopifyId', 'erpId', 'status', 'title', 'variants'
                            ]
                            let variantNeededKeys: Array<keyof InnovateVariantInfo> = [
                                'compareAtPrice', 'shopifyId', 'price', 'sku'
                            ]
                            ...
                            if ( !!_product.variants && Array.isArray(_product.variants) ) {
                                _product.variants.forEach(variant => {
                                    Object.keys(variant).forEach((k: keyof InnovateVariantInfo) => {
                        ...
                        if ( task.type === TaskType.UPDATE_STOCK ) {
                            let prodNeededKeys: Array<keyof InnovateProductInfo> = [
                                'shopifyId', 'erpId', 'variants', 'title'
                            ]
                            let variantNeededKeys: Array<keyof InnovateVariantInfo> = [
                                'shopifyId', 'inventory', 'erpId', 'sku'
                            ]
                            ...
                            if ( !!_product.variants && Array.isArray(_product.variants) ) {
                                _product.variants.forEach(variant => {
                                    ...
                                    variant.inventory = variant.inventory.filter(l => 
                                        shopifyLocations.some(_l => _l.id === l.locationId && _l.active)
                                    )
                                })
                            }
                        }
                    ...
                    // LOG ERROR ON PRODUCT
                    pInfo.observations = err.response || `ERROR WHILE UPDATING PREV INFO | ${err.message}`
                    pInfo.status = false
                    result.failed++
                    pInfo.processedAt = Date.now()
                    result.results.push(pInfo)
                ...
                try {
                    if ( _product.metafields?.length > 0 && !!thisProduct ) {
                        const productMetafields = await ShopifyProvider.queueRestRequest<RestResources['Metafield'][]>(async () => 
                        ...
                        for await ( const metafield of _product.metafields ) {
                            const existMeta = productMetafields.find(m => 
                    ...
                    // LOG ERROR ON PRODUCT
                    pInfo.observations = err.response || 'ERROR WHILE UPDATING METAFIELDS'
                    pInfo.status = false
                    result.failed++
                    pInfo.processedAt = Date.now()
                    result.results.push(pInfo)
                ...
                if ( _ShopifyProvider.GRAPHQL_API_SLEEP_COUNT > 0 ) { // FOR SPECIAL CASES
                    await HelpersController.sleep(_ShopifyProvider.GRAPHQL_API_SLEEP_COUNT)
                }

                if ( _ShopifyProvider.GRAPHQL_API_AVAILABLE_CREDITS <= _ShopifyProvider.GRAPHQL_API_MINIMUM_SAFE ) {
                    const fullyRestoreCredits = _ShopifyProvider.GRAPHQL_API_MAX_AVAILABLE_CREDITS - _ShopifyProvider.GRAPHQL_API_AVAILABLE_CREDITS
                ...
                if ( i % _ShopifyProvider.GRAPHQL_API_PARALLEL_REQUESTS === 0 ) {
                    ...
                    try {
                        ...
                        _ShopifyProvider.GRAPHQL_API_AVAILABLE_CREDITS = cost.throttleStatus.currentlyAvailable
                        ...
                        _ShopifyProvider.GRAPHQL_API_MAX_AVAILABLE_CREDITS = initialTotalCredits
                        // ON SYNCHRONOUS REQUESTS RE-CHECK FOR API STATUS

                        pInfo.apiHealthData = cost
                        if ( !!userErrors && userErrors.length > 0 ) throw { response: userErrors }
            ...
            isError = true
            await task.updateOne({ $set: { state: TaskState.ERROR, observations: 'TASK GLOBAL ERROR' } })
            console.log('ERROR THREAD PRODUCT TASKS!')
            console.log(err.message)
            setTimeout(() => { throw { error: err.message } }, 150)
        }
