...
class AgilisaProvider {
    static PROVIDER_CALLS = 0;
    IS_SANDBOX_PROXY = false;
    PROXY_SV_DOMAIN = process.env.PROXY_SV_DOMAIN || '';
    SV_DOMAIN;
    DB_NAME;
    DB_PW;
    DB_USER;
    ACTIVE_BRAND_IDS = [];
    RETRIEVE_ONLY_AVAILABLE_STOCK = true;
    constructor() {
        this.setup(false);
    ...
    async setup(isOrderQuery = false) {
    ...
    async execSqlCmd({ query, inputs, isOrderQuery = false }) {
    ...
    async execQueryOnProxyServer({ query }) {
    ...
    /* ------------------- END PRIVATE */
    async getAllMasterProducts({ limit = 2500, fields = [] }) {
        await this.setup();
    ...
    async getMasterBrands({ fields = [] } = {}) {
        await this.setup();
        const _fields = fields.map((x) => `[${x}]`).join(',');
        let query = `
            SELECT 
    ...
    async getMasterProducts({ limit = 2500, fields = [], page = 1, all = false, where = '', allBrands = false }) {
        await this.setup();
        const _fields = fields.map((x) => `[${x}]`).join(',');
        if (!!!where && !allBrands) {
    ...
    async getMasterProductsCount({ all = false } = {}) {
        await this.setup();
    ...
    async getMasterProductById(lookupId) {
        await this.setup();
        let query = `
            SELECT 
            *
    ...
    async getProductVariantsByRef(reference) {
        await this.setup();
        let query = `
            SELECT 
            *
    ...
    async getAllMasterInventory({ limit = 2500, fields = [] }) {
        await this.setup();
    ...
    async getMasterInventory({ limit = 2500, fields = [], page = 1, all = false, where = '', allBrands = false }) {
        const _fields = fields.map((x) => `[${x}]`).join(',');
        if (!!!where && !allBrands) {
            where = `WHERE Brand_Id IN (${this.ACTIVE_BRAND_IDS.join(',')})`;
        }
    ...
    async getMasterInventoryCount({ all = false } = {}) {
        await this.setup();
        let where = all ? '' : `WHERE Brand_Id IN (${this.ACTIVE_BRAND_IDS.join(',')})`;
        if (this.RETRIEVE_ONLY_AVAILABLE_STOCK) {
            if (!!where)
    ...
    async getMasterInventoryById(lookupId, fields = []) {
        await this.setup();
        const _fields = fields.map((x) => `[${x}]`).join(',');
        let query = `
            SELECT 
    ...
    async getProductVariantsInventoryByRef(reference) {
        await this.setup();
        let query = `
            SELECT 
            *
    ...
    async insertNewOrderInfo(fields = []) {
    ...
    async insertNewOrderProductDetails(fields = []) {
        await this.setup(true);
        const NameType = {
            OrderNumber: String,
    ...
    async insertNewOrder(orderId) {
        ...
        if (!!!locationId) {
            const fulfillmentOrders = await ShopifyProvider.queueRestRequest(async () => await ShopifyProvider.getFulfillmentOrdersByOrderId(order.id));
        ...
        const brand = brands.filter(x => activeBrands.map(b => String(b)).includes(String(x.Brand_Id)));
    ...
    async deleteOrderHeaderById(id) {
        await this.setup(true);
        let query = `
            DELETE 
            FROM [${this.DB_NAME}].[dbo].[Shopify_Order]
    ...
    async deleteOrderDetailsById(id) {
        await this.setup(true);
        let query = `
            DELETE 
            FROM [${this.DB_NAME}].[dbo].[Shopify_OrderDetail]
