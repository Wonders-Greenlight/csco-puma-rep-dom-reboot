import ShopifyProvider, { shopify } from '../providers/ShopifyProvider.js';
import { randomUUID } from 'crypto';
import TaskProvider, { TASKS } from "../providers/TaskProvider.js";
import { TaskPriority, TaskType } from "../interfaces/TaskInterfaces.js";
...
class ShopifyController {
    static async loginIntoApp(req, res, next) {
        ...
        const session = res.locals.shopify?.session ||
            await ShopifyProvider.getSessionFromStorage(onlineSessionId);
    ...
    static async refreshSessionToken(req, res) {
        console.log('checkToken |no errors yet line 53');
        const refreshToken = req.headers['x-innovate-refresh-token'];
    ...
    static async getProductsCount(req, res) {
        ...
        const offlineSessionId = shopify.api.session.getOfflineId(process.env.SHOP || 'puma-rep-dominicana.myshopify.com');
    ...
    static async getVariantsCount(req, res) {
        // console.log(req.headers);
    ...
    static async getProducts(req, res) {
        ...
        const offlineSessionId = shopify.api.session.getOfflineId(process.env.SHOP);
        // use sessionId to retrieve session from app's session storage
        // getSessionFromStorage() must be provided by application
        console.log('\n\nHERE SESSION!!!!');
        console.log(onlineSessionId);
    ...
    static async getInventoryLevelsByLocation(req, res) {
    ...
    static async getAllInventoryLevels(req, res) {
        const { inventory_item_ids, limit, location_ids } = req.query;
        try {
    ...
    static async getInventoryItems(req, res) {
        ...
        if (!!!ids)
            return res.status(400).json({ message: 'Please send "ids" query parameter, with comma separated inventory item ids' });
        try {
    ...
    static async setInventoryLevel(req, res) {
        const { id } = req.params;
    ...
    static async adjustInventoryLevel(req, res) {
        const { id } = req.params;
    ...
    static async setVariantPrice(req, res) {
        const { id } = req.params;
    ...
    static async setVariantPriceOnProduct(req, res) {
    ...
    static async getCarrierServices(req, res) {
        try {
    ...
    static async getCarrierServiceById(req, res) {
        try {
            const { id } = req.params;
    ...
    static async deleteCarrierService(req, res) {
        try {
            const { id } = req.params;
    ...
    static async updateCarrierService(req, res) {
        try {
            const { id } = req.params;
            const { name, callback_url } = req.body;
            const carrierService = await ShopifyProvider.getCarrierServiceById(Number(id), false);
            if (!!!carrierService)
                return res.status(404).json({ message: 'Carrier Service with given ID not found' });
    ...
    static async createCarrierService(req, res) {
        try {
    ...
    static async createProductGql(req, res) {
        try {
            // Creating/Updating payload should be taken from request body, this
            // is an example
            const erpProducts = [];
    ...
    static async updateProductGql(req, res) {
        try {
            const erpProducts = [];
            erpProducts.push({
                erpId: 7772337569942,
    ...
    static async getProductGql(req, res) {
        try {
            const product = await ShopifyProvider.gqlGetProductsByQuery(req.query.query);
            return res.json(product);
        }
        catch (err) {
            res.status(500).json({
    ...
    static async bulkInventoryUpdate(req, res) {
        try {
