...
import { 
    TaskEntry, TTask, TaskPriority, TaskFn, 
    TaskState, TaskType, ITask 
} from '../interfaces/TaskInterfaces.js';
import { ObjectValues } from '../interfaces/AppInterfaces.js'
...
export const TASKFILE_BY_TYPE: { [key: string]: string } = {
...
export const TASK_FUNCTION: { [key: string]: string } = {
...
class _TaskProvider {
    private SHOW_LOGS: boolean = true;
    public cronTab: cron.ScheduledTask[] = [];
    public queue: Array<TTask>;
    public activeWorkers: { [key: string]: Worker } = {}
    ...
    private setQueueProxy() {
        ...
        this.queue = new Proxy([], {
            ...
            set: (target, property, value, receiver) => {
                ...
                if ( this.queue.some(x => !!x.processing) ) return true
                this.runNextTask()
                return true;
            }
        });
    ...
    private log( toBeLogged: any ) {
        if ( !this.SHOW_LOGS ) return
    ...
    private checkForParallelTask() {
        ...
        const highPriorityDistincTasks = this.queue.reduce((acc, t) => {
            ...
            if ( t.priority !== TaskPriority.HIGH || existType ) return acc
            if ( processingTasks.some(_t => _t.id === t.id || _t.type === t.type) ) return acc
            acc.push(t)

            return acc
        }, [])

        if ( !!!highPriorityDistincTasks.length ) return
    ...
    private async checkFrozenTasks( nowDate: Date ) {
        try {
            ...
            if ( !!!processingTaskDb ) return
            const nowTime = new Date() as any
            const startedTime = new Date(processingTaskDb.startedAt) as any
            
            const restTime = new Date(nowTime - startedTime)
            if ( restTime.getMinutes() < 10 ) return
    ...
    private async runCron() {
        ...
        if ( cfg?.cronPriceUpdateRoutineEnabled ) {
            const automatedUpdatePrice = cron.schedule(
                `*/${cfg.cronMinutesPriceUpdateTask} * * * *`, () => {
        ...
        if ( cfg?.cronStockUpdateRoutineEnabled ) {
            const automatedUpdateStock = cron.schedule(
                `*/${cfg.cronMinutesStockUpdateTask} * * * *`, () => {
        ...
        if ( cfg?.cronProductUpdateRoutineEnabled ) {
            const automatedProductUpdateStock = cron.schedule(
                `*/${cfg.cronMinutesProductCreateUpdateTask} * * * *`, () => {
        ...
        const automatedProductUpdateStockEvery4hs = cron.schedule(
            `30 10-20/4 * * *`, () => {
                TaskController.createTaskByType.bind(TaskController)(TaskType.CREATE_UPDATE_PRODUCTS)
            },
            options
    ...
    public async resumeOnHoldTasks() {
        ...
        const parsedTasks: TTask[] = pendingTasks
    ...
    public async stopProcessingHoldTasks() {
    ...
    public async deleteCancelledTasks() {
        const now = new Date()
    ...
    public buildTaskPayloadFromDbTask( t: ITask ) {
        return {
            id: t.busId,
            processing: false,
    ...
    public queueTaskInPlace( t: TTask ) {
        if ( t.priority === TaskPriority.LOW ) {
            this.queue.push(t)
        } else {
            const firstHigherIndex = this.queue.findIndex(x => 
                x.priority === t.priority + 1
            )
    ...
    public async add( task: TaskEntry | TaskFn, priority: TaskPriority = TaskPriority.NORMAL ) {
        const isMainTask = typeof task === 'function'

        let taskPayload: TTask = {
            id: randomUUID(),
            processing: false,
            runOnSeparateThread: true,
        ...
        if ( isMainTask ) {
            ...
            if ( !!!taskPayload.filePath ) taskPayload.filePath = TASKFILE_BY_TYPE[taskPayload.type]
            if ( !!!taskPayload.fnToCall ) taskPayload.fnToCall = TASK_FUNCTION[taskPayload.type]
            ...
            try {
                ...
                if ( !!task.eventId ) dbTask.eventId = task.eventId as any // string ID
                if ( !!task.schedule ) {
        ...
        if ( !!taskPayload.schedule ) return taskPayload
    ...
    public runTask({ 
    ...
    }): Promise<{ pId: string } | any> {
        if ( !!!taskFile ) taskFile = TASKFILE_BY_TYPE[type as keyof typeof TASKFILE_BY_TYPE]
        if ( !!!fnToCall ) fnToCall = TASK_FUNCTION[type as keyof typeof TASK_FUNCTION]
        ...
        return new Promise(async (res, rej) => {
            ...
            try {
                const worker = new Worker(taskFile as string, {
                    workerData: {
                        fn: fnToCall,
                        args,
                        pId
                ...
                if ( !!thisTask ) thisTask.worker = worker 
                ...
                worker.on('message', (data) => {
                    ...
                    worker.terminate()
                    if ( !!data.isError ) return rej({ ...processInfo, error: data })
    ...
    public cleanTempStaticFolder( task: TTask, seconds: number = 5 ) {
        setTimeout(() => {
            const staticTaskPath = path.resolve(folders.TEMP, task.id)
            if ( !fs.existsSync(staticTaskPath) ) return
            fs.rmSync(staticTaskPath, { recursive: true, force: true })
        }, seconds * 1000)
    }

    public async runNextTask( nextTaskIndex: number = 0 ): Promise<void> {
        ...
        if ( nextTask.runOnSeparateThread ) {
            ...
            SocketProvider.emitEventToRoom(
                '/tasks', 
                'taskUpdate',
                await Task.findOne({ busId: nextTask.id }).select('-outerData')
            )
            ...
            try {
                ...
                await Task.updateOne({ busId: pId || nextTask.id }, {
                    $set: {
                        ...
                        finishedAt: new Date(),
                        outerData: result,
                        microstateStatus: !!!result?.results?.some((x: any) => !x.status)
                    }
                })
        ...
        if ( !!dbTask ) {
            ...
            if ( !!dbTask?.eventId ) {
                const eventData = { ...(dbTask.eventId as any).toJSON() }
